...
for (int index_y=0; index_y<bob->height; index_y++)
    ...
    // advance pointers
    dest_ptr   += pitch;////RILEY (bob->width+bob->width_fill)
    source_ptr += bitmap->bitmapinfoheader.biWidth;
    } // end for index_y

// unlock the surface 
(bob->images[frame])->Unlock((struct tagRECT *)ddsd.lpSurface);
...
lpdds->Unlock((struct tagRECT *)surface_buffer);
...
lpddsprimary->Unlock((struct tagRECT *)primary_buffer);
...
lpddsback->Unlock((struct tagRECT *)back_buffer);
